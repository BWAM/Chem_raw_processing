...
Copy "RIBS_EDD_processing_MASTER.Rmd" from root folder into project-specific folder in data_output and rename/modify as needed.
...
input.path <- file.path(root.dir, "data_input", "2020")
output.path <- file.path(root.dir, "data_output", "2020", "TEST")
output.filename <- paste0("20202_tabseptest_", Sys.Date(), "TEST.csv")
...
for (i in 1:nfolder_list) {
  ...
  ## Alternative method #2 to generating SiteID. Pulls string from before first underscore
  # temp_sample$SiteID <- ifelse(temp_sample$sample_source == "field" & temp_sample$sample_type_code == "N",
  #                            str_extract(temp_sample$sample_name, "[^_]+"), NA)
  
  ## Alternative method #3 to generating SiteID. Pulls string from position after decimal (as expected in river mile). WOrks better for older data:
  ...
  temp_sample <- temp_sample %>% 
    mutate(DEC_sample_type2 = case_when(
      ...
      sample_source == "Field" &
        sample_type_code == "N" &
        grepl(
          "-[Dd][Uu][Pp]|W[Dd][Uu][Pp]|WS[Dd][Uu][Pp]|[Dd][Uu][Pp]_| [Dd][Uu][Pp]",
          sample_name
        ) ~ "DUP",
