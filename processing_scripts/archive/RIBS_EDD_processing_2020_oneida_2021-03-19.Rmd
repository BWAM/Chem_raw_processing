...
input.path <- file.path(root.dir, "data_input", "2020", "oneida")
output.path <- file.path(root.dir, "data_output", "2020", "oneida")

output.filename <- paste0("2020_chem_preqaqc_ALL-oneida_", Sys.Date(), ".csv")
output.sample.filename <- paste0("2020_chem_preqaqc_SAMPLE-oneida_", Sys.Date(), ".csv")
output.result.filename <- paste0("2020_chem_preqaqc_RESULT-oneida_", Sys.Date(), ".csv")
output.batch.filename <- paste0("2020_chem_preqaqc_BATCH-oneida_", Sys.Date(), ".csv")
...
for (i in 1:nfolder_list){
  ...
  # Using sys_loc_code in leiu of pulling from sample_name (above block)
  temp_sample <- temp_sample %>% 
    mutate(SITE_ID = ifelse(grepl("-", sys_loc_code), sys_loc_code, "LAB_INTERNAL")) %>% 
