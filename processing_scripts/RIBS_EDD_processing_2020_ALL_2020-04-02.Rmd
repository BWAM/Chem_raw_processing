...
# Import 2020 sites and join to sites master for a unique list
sites.2020 <- read_excel(file.path(root.dir, "fielddata_for_crosscheck/20210309_S_Sample_Event_Info_field_all_fields_Charlie-2021-03-20.xlsx")) %>%
  rename(BAS_LOC_RM = SEIH_EVENT_SMAS_HISTORY_ID) %>% 
  distinct(BAS_LOC_RM)
sites.master <- sites.master %>% 
  bind_rows(sites.2020) %>% 
  distinct(BAS_LOC_RM)
...
RIBSdata.ALL_corr <- RIBSdata.ALL %>% 
  mutate(SITE_ID_corr = case_when(
    ...
    SITE_ID == '15-RAMA-' ~ '15-RAMA-16.8',  #ALS error; Extra dash added.
    SITE_ID == '13WALK-60.1-07072020' ~ '13-WALK-60.1',
    SITE_ID == '13-LMOHE-P' ~ '13-LMOHE-0.0',  # Updated to match LMAS site ID. 
    SITE_ID == '13-LPEAC-P' ~ '13-LPEAC-0.0',  # Updated to match LMAS site ID. 
    SITE_ID == '13-LPNAM-P' ~ '13-LPNAM-0.0',  # Updated to match LMAS site ID. 
    SITE_ID == '07-WAGGS-0.1' ~ '07-WAGG-0.1',  # ALS error; Extra 'S' added. 
    SITE_ID == '03-SKIN_T5-0.5' ~ '03-SKIN_T5-0.5', # ALS error; RM 0.2 on COC. 
