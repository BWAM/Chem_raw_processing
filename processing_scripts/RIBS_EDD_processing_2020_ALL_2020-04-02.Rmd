...
input.path <- file.path(root.dir, "data_input", "2020", "all")
output.path <- file.path(root.dir, "data_output", "2020", "all")

output.filename <- paste0("2020_chem_preqaqc_JOIN-ALL_", Sys.Date(), ".csv")
output.sample.filename <- paste0("2020_chem_preqaqc_SAMPLE-ALL_", Sys.Date(), ".csv")
output.result.filename <- paste0("2020_chem_preqaqc_RESULT-ALL_", Sys.Date(), ".csv")
output.batch.filename <- paste0("2020_chem_preqaqc_BATCH-ALL_", Sys.Date(), ".csv")
...
sites.master <- read_excel(file.path(root.dir, "data_input/site_tables/2020_05_01_S_Site.xlsx")) %>% 
...
als.proj.list <- read_excel(file.path("L:/DOW/SMAS/data/chemistry/raw_lab_edds/2020", "nysdec 2020_Janice_ALS_proj_list_2021-02-12.xlsx"), sheet = "SMAS") %>% 
  janitor::clean_names() %>% 
  filter(project_dec != "ESF CREEQ") %>% 
  rename(SDG = folder_number)

edds.missing <- anti_join(edds.present, als.proj.list, by = "SDG")
edds.missing.2 <- anti_join(als.proj.list, edds.present, by = "SDG")

# ALL PRESENT EXCEPT R2000546, which is for sediment samples taken by a region (Rondout Creek, Rosen/RRS 19)
...
for (i in 1:nfolder_list){
  ...
  temp_sample$SITE_ID <- ifelse(temp_sample$sample_matrix_code == "WS|SO",
  ...
  dup.vec <- temp_sample$sample_name[grepl("-[Ss][Ee][Qq]", temp_sample$sample_name)]
  ...
  dup.vec <- temp_sample$sample_name[grepl("WS[Ss][Ee][Qq]", temp_sample$sample_name)]
  parent.vec <- gsub("WS[Ss][Ee][Qq]", "WS", dup.vec)
  temp_sample$DEC_sample_type <- ifelse(temp_sample$sample_name %in% parent.vec,"N_DUPPARENT",temp_sample$DEC_sample_type)
  
  dup.vec <- temp_sample$sample_name[grepl("W[Ss][Ee][Qq]", temp_sample$sample_name)]
  parent.vec <- gsub("W[Ss][Ee][Qq]", "W", dup.vec)
  temp_sample$DEC_sample_type <- ifelse(temp_sample$sample_name %in% parent.vec,"N_DUPPARENT",temp_sample$DEC_sample_type)

  dup.vec <- temp_sample$sample_name[grepl("[Ss][Ee][Qq]_", temp_sample$sample_name)]
...
RIBSdata.ALL_corr <- RIBSdata.ALL %>% 
  mutate(SITE_ID_corr = case_when(
    ...
    SITE_ID == '15-RAMA-' ~ '15-RAMA-16.8',
    SITE_ID == '13WALK-60.1-07072020' ~ '13-WALK-60.1',
    SITE_ID == '07-WAGGS-0.1' ~ '07-WAGG-0.1',
    SITE_ID == '13-WALK-19.0_090120' ~ '13-WALK-19.0',
    SITE_ID == '13-WALK-44.4_090120' ~ '13-WALK-44.4',
    SITE_ID == '13-WALK-60.1_090120' ~ '13-WALK-60.1',
    TRUE ~ SITE_ID
  )
  ) %>% 
...
Subset data to Charlie's site list (those with CREEQ surveys)
```{r, eval = FALSE}
crsurvey.sites <- readxl::read_excel(file.path(root.dir, "data_input/2020/20210212_Edit_Filtered_DEC_Creeq_UP_events.xlsx")) %>% 
  distinct(SITE_HISTORY_ID)

RIBSdata.ALL_corr.crsurvey <- RIBSdata.ALL_corr %>% 
  filter(SITE_ID %in% crsurvey.sites$SITE_HISTORY_ID,
         DEC_sample_type %in% "N",
         sample_source %in% "Field") %>% 
  select(SITE_ID, sys_sample_code, sample_delivery_group, sample_date, fraction, chemical_name, result_value, result_unit, reportable_result, detect_flag, lab_qualifiers, interpreted_qualifiers, method_detection_limit, reporting_detection_limit, quantitation_limit, detection_limit_unit, minimum_detectable_conc)

RIBSdata.ALL_corr.crsurvey.unique <- lapply(RIBSdata.ALL_corr.crsurvey, unique)

write.table(RIBSdata.ALL_corr.crsurvey, file= paste0(output.path,"/","2020_chem_preqaqc_CREEQ_REQUEST_", Sys.Date(), ".csv"),sep=",", row.names = FALSE)
