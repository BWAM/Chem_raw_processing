...
proj_name <- "pcbs_2017-2019"    # use only lowercase with underscores as spaces"
...
for (i in 1:nfolder_list){
  ...
  temp_sample <- temp_sample %>% 
    mutate(sample_matrix_code = ifelse(is.na(sample_matrix_code), ifelse(sample_source %in% "Field", "WS", "WQ"),sample_matrix_code))
...
RIBSdata.ALL_corr <- RIBSdata.ALL %>% 
  mutate(SITE_ID_corr = case_when(
    SITE_ID == '03EMIL-5.1-EB_08142017_WS' ~ '03-EMIL-5.1',
    SITE_ID == '03EMIL-5.1_08142017_WS' ~ '03-EMIL-5.1',
    SITE_ID == '00-QAQC-0.0' ~ '00-QAQC-0.0',
    SITE_ID == '03EMIL-5.1-DUP_10172017_AW' ~ '03-EMIL-5.1',
    SITE_ID == '03EMIL-5.1-FB_10172017_WS' ~ '03-EMIL-5.1',
    SITE_ID == '03EMIL-5.1_10172017_AW' ~ '03-EMIL-5.1',
    TRUE ~ SITE_ID
  )
  ) %>% 
...
# lab_matrix_code (a non-null field in the DB) is blank for 2018 EDDs. Join from sample file.

RIBSdata.sample_corr_matrix <- RIBSdata.sample_corr %>% 
  rename(lab_sdg = sample_delivery_group) %>% 
  select(sys_sample_code, lab_sdg, sample_matrix_code)

RIBSdata.result_fix <- RIBSdata.result %>% 
  left_join(RIBSdata.sample_corr_matrix) %>% 
  mutate(lab_matrix_code = ifelse(is.na(lab_matrix_code), sample_matrix_code, lab_matrix_code)) %>% 
  select(-sample_matrix_code)
...
write.table(RIBSdata.result_fix, file= paste0(output.path,"/",output.result.filename),sep=",", row.names = FALSE)
