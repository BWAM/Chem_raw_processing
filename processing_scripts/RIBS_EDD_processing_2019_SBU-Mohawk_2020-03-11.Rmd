...
input.path <- file.path(root.dir, "data_input", "2019", "all_SBU_Mohawk_complete")
output.path <- file.path(root.dir, "data_output", "2019", "all_SBU_Mohawk_complete")
output.filename <- paste0("2019_chem-ALL_SBU_Mohawk_pre-qaqc_", Sys.Date(), ".csv")
output.sample.filename <- paste0("2019_chem-SAMPLE_SBU_Mohawk_pre-qaqc_", Sys.Date(), ".csv")
output.result.filename <- paste0("2019_chem-RESULT_SBU_Mohawk_pre-qaqc_", Sys.Date(), ".csv")
output.batch.filename <- paste0("2019_chem-BATCH_SBU_Mohawk_pre-qaqc_", Sys.Date(), ".csv")
...
2) Add SITE_ID and DEC_sample_type fields to each Sample_v3.
...
# Commented out blocks below only look at field and sample type N   
...
# Now applying codes to all data (including lab samples; MS, MSD, LR)
  ...
  # Populate DEC_sample_type for FB samples.
  ...
  # Reworking for new DEC_sample_type coding
  count.Normal <- length(temp_sample$DEC_sample_type[temp_sample$sample_source == "Field" & temp_sample$sample_type_code == "N"
                                     & grepl("^N$", temp_sample$DEC_sample_type)])  
  message(paste0(count.Normal," N"))
  count.N_DUPPARENT <- length(temp_sample$DEC_sample_type[temp_sample$sample_source == "Field" & temp_sample$sample_type_code == "N"
                                     & grepl("N_DUPPARENT", temp_sample$DEC_sample_type)])  

  message(paste0(count.N_DUPPARENT," N_DUPPARENT"))
  count.DUP <- length(temp_sample$DEC_sample_type[temp_sample$sample_source == "Field" & temp_sample$sample_type_code == "N"
                                     & grepl("^DUP$", temp_sample$DEC_sample_type)])  
  message(paste0(count.DUP," DUP"))
  count.EB <- length(temp_sample$DEC_sample_type[temp_sample$sample_source == "Field" & temp_sample$sample_type_code == "N"
                                     & grepl("^EB$", temp_sample$DEC_sample_type)])  
  message(paste0(count.EB," EB"))
  count.FB <- length(temp_sample$DEC_sample_type[temp_sample$sample_source == "Field" & temp_sample$sample_type_code == "N"
                                     & grepl("^FB$", temp_sample$DEC_sample_type)])  
  message(paste0(count.FB," FB\n"))
...
totalcount.Normal <- length(RIBSdata.sample$DEC_sample_type[RIBSdata.sample$sample_source == "Field" & RIBSdata.sample$sample_type_code == "N"
                                     & grepl("^N$", RIBSdata.sample$DEC_sample_type)])
totalcount.DUP <- length(RIBSdata.sample$DEC_sample_type[RIBSdata.sample$sample_source == "Field" & RIBSdata.sample$sample_type_code == "N"
                                     & grepl("^DUP$", RIBSdata.sample$DEC_sample_type)])
totalcount.N_DUPPARENT <- length(RIBSdata.sample$DEC_sample_type[RIBSdata.sample$sample_source == "Field" & RIBSdata.sample$sample_type_code == "N"
                                     & grepl("N_DUPPARENT", RIBSdata.sample$DEC_sample_type)])
totalcount.EB <- length(RIBSdata.sample$DEC_sample_type[RIBSdata.sample$sample_source == "Field" & RIBSdata.sample$sample_type_code == "N"
                                     & grepl("^EB$", RIBSdata.sample$DEC_sample_type)])
totalcount.FB <- length(RIBSdata.sample$DEC_sample_type[RIBSdata.sample$sample_source == "Field" & RIBSdata.sample$sample_type_code == "N"
                                     & grepl("^FB$", RIBSdata.sample$DEC_sample_type)])     # FB only present in Routine Network data
...
# ifelse(is.character(substring(RIBSdata.ALL$SITE_ID, 3, 3)),stringi::stri_sub(RIBSdata.ALL$SITE_ID, 3, 2) <- "-",RIBSdata.ALL$SITE_ID)
# ifelse(is.character(substring(RIBSdata.ALL$SITE_ID, 3, 3)),gsub('^([a-z]{3})([a-z]+)$', '\\1-\\2', old), NA)

# Return string before underscore
# RIBSdata.ALL$SITE_ID <- gsub( "_.*$", "", RIBSdata.ALL$SITE_ID)

# RIBSdata.ALL$SITE_ID <- substr(RIBSdata.ALL$SITE_ID, 1, as.numeric(gregexpr(pattern ='\\.',RIBSdata.ALL$SITE_ID)) + 1)
...
Create additional fields, format fields appopriately
# NEED TO MOVE THIS REFORMATTING TO A NEW SCRIPT, or do it after running QAQC, since that required fields to be formatted as-as. 
```{r}

# Modify full joined table as needed
RIBSdata.ALL_corr_TEST <- RIBSdata.ALL_corr %>% 
  rename_all(toupper) %>% 
  mutate(EVENT_ID = paste0(SITE_ID,"_",as.Date(SAMPLE_DATE, format = "%m/%d/%Y"))) %>% 
  select(EVENT_ID, everything())

# Modify SAMPLE table as needed
RIBSdata.sample_corr_TEST <- RIBSdata.sample_corr %>% 
  rename_all(toupper) %>% 
  mutate(EVENT_ID = ifelse(!is.na(SITE_ID), paste0(SITE_ID,"_",as.Date(SAMPLE_DATE, format = "%m/%d/%Y")), SITE_ID)) %>% 
  select(EVENT_ID, everything()) %>% 
...
# Modify RESULT table as needed
RIBSdata.result_TEST <- RIBSdata.result %>% 
  rename_all(toupper) %>% 
  mutate(PARAMETER_ID = NA)
  # format dates, numbers, etc.

# Modify BATCH table as needed
RIBSdata.batch_TEST <- RIBSdata.batch %>% 
  rename_all(toupper)
  # format dates, etc.

# Format all dates correctly, number fields, etc
### STOP SCRIPT IF Hg EBs exist
...
Test filtering to redesign Site ID and sample type assignments
```{r}
RIBSdata.sample.WQ <- RIBSdata.sample_corr %>% 
...
# Join by both SSC and SDG
 test_join_SSC_SDG <- left_join(RIBSdata.sample_corr, RIBSdata.result, by = c("sys_sample_code", "lab_sdg"))
...
```{r}
chem_params <- unique(RIBSdata.ALL_corr[c("fraction", "cas_rn", "chemical_name","result_unit")])

write.table(chem_params, file= paste0(output.path,"/","chem_params.csv"),sep=",", row.names = FALSE)
