...
input.path <- file.path("C:/Users/gmlemley/New York State Office of Information Technology Services/BWAM - TestAmerica/2021/routine_pfas/")
...
for (i in 1:nfolder_list){
  ...
  temp_result <- read_excel(dir(input.i, full.names=T, pattern="xls"), sheet = "TestResultQC_v4") %>% 
    rename(sys_sample_code = "#sys_sample_code")

  temp_sample <- read_excel(dir(input.i, full.names=T, pattern="xls"), sheet = "Sample_v4") %>% 
