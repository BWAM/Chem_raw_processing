...
sample.ribs <- sample.proj %>% 
  filter(grepl("Routine_CECs", DEC_project))
...
sample.meth <- sample.proj %>% 
  filter(grepl("CEC_methods_eval", DEC_project))

sample.pfaspw.apr <- sample.proj %>% 
  filter(grepl("PFAS_priority_waters", DEC_project),
...
Apply project names and write final tables to CSV 
```{r}
