...
input.path <- file.path(root.dir, "data_input", "2020", "canan_keuka")
output.path <- file.path(root.dir, "data_output", "2020", "canan_keuka")

output.filename <- paste0("2020_chem_preqaqc_ALL-canan_keuka_", Sys.Date(), ".csv")
output.sample.filename <- paste0("2020_chem_preqaqc_SAMPLE-canan_keuka_", Sys.Date(), ".csv")
output.result.filename <- paste0("2020_chem_preqaqc_RESULT-canan_keuka_", Sys.Date(), ".csv")
output.batch.filename <- paste0("2020_chem_preqaqc_BATCH-canan_keuka_", Sys.Date(), ".csv")
...
for (i in 1:nfolder_list){
  ...
  temp_result <- read.table(text = gsub("\t",",", readLines(file.path(input.i,"TestResultQC_v3.txt"))), skip = 1,
  ...
  temp_sample <- read.table(text = gsub("\t",",", readLines(file.path(input.i,"Sample_v3.txt"))), skip = 1,
                                      sep=",",fill=TRUE,header=FALSE,stringsAsFactors=FALSE,
  ...
  temp_batch <- read.table(text = gsub("\t",",", readLines(file.path(input.i,"Batch_v3.txt"))), skip = 1,
                                      sep=",",fill=TRUE,header=FALSE,stringsAsFactors=FALSE,
