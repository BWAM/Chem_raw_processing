...
input.path <- file.path(root.dir, "data_input", "2021", "mohawk_tss")
output.path <- file.path(root.dir, "data_output", "2021", "mohawk_tss")

output.filename <- paste0("2021_chem_preqaqc_JOIN-mohk_tss-PARTIAL_", Sys.Date(), ".csv")
output.sample.filename <- paste0("2021_chem_preqaqc_SAMPLE-mohk_tss-PARTIAL_", Sys.Date(), ".csv")
output.result.filename <- paste0("2021_chem_preqaqc_RESULT-mohk_tss-PARTIAL_", Sys.Date(), ".csv")
output.batch.filename <- paste0("2021_chem_preqaqc_BATCH-mohk_tss-PARTIAL_", Sys.Date(), ".csv")
...
RIBSdata.ALL_corr <- RIBSdata.ALL %>% 
  mutate(SITE_ID_corr = case_when(
    ...
    SITE_ID == '12-SCHO-1.2' ~ '12-SCHO-1.2',    TRUE ~ SITE_ID
  )
  ) %>% 
