...
input.path <- file.path(root.dir, "data_input", "2018", "Ramapo")
output.path <- file.path(root.dir, "data_output", "2018", "Ramapo")
output.filename <- "2018_Ramapo_chem_raw_2-15-19.csv"
...
for (i in 1:nfolder_list){
  ...
  temp_result$fraction <- plyr::revalue(temp_result$fraction, c("TRUE" = "T"), warn_missing = FALSE)
...
### Move this check to after binding data, see how many DUPS, DUP parents, EBs (MSs?) exist. Need to subset data and look at DEC_sample_type.
  # Okay if single SDGs don't have certain QAQC samples, but give warning if full set is missing any (and give total count).
    ...
    warning("    WARNING: NO DUPLICATES PRESENT\n")
  }
...
# There were several 'fraction' values for aluminum that should have been D for dissolved and not T.
