...
input.path <- file.path(root.dir, "data_input", "2018", "FingerLakes")
output.path <- file.path(root.dir, "data_output", "2018", "FingerLakes")
output.filename <- "2018_FingerLakes_chem_raw_2-19-19.csv"
# DO NOT RUN FULL SCRIPT. MUST RUN CHUNK-BY-CHUNK AND CORRECT SITE IDs AND DATA WHERE NEEDED #
...
### ACCOUNT FOR UPPER AND LOWER CASE in below (see QAQC.Rmd) ###
### ACCOUNT FOR DUPs labeled as "QC"... doesn't the new QAPP say to use this??
  ...
  # Count number of DUP and N_DUPPARENT codes assigned and make sure they're equal
  dup.count <- length(grep("^DUP$", temp_sample$DEC_sample_type))
  dupparent.count <- length(grep("N_DUPPARENT", temp_sample$DEC_sample_type))
  
  if(identical(dup.count,dupparent.count) == FALSE){
    stop("    DUP and N_DUPPARENT counts do not match (script stopped)")
  }

### Move this check to after binding data, see how many DUPS, DUP parents, EBs (MSs?) exist. Need to subset data and look at DEC_sample_type.
  # Okay if single SDGs don't have certain QAQC samples, but give warning if full set is missing any (and give total count).
    if(dup.count == 0 | dupparent.count == 0){
    warning(paste0("    WARNING: NO DUPLICATES PRESENT in ",folder_list[i],"\n"))
  }
  ...
  # Do the same for only sample file data (for troubleshooting group EDD issues)
...
###
### Add check here for summarizing QAQC samples (inluding total sample count) for user to compare to COCs (to make sure all assigned appropriately).
###
...
if (length(siteids.mismatch) == 0 & length(siteids.match) > 0) {
  print("Match found all site IDs.")
} else{
